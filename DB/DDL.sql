...
create index quest_space_user_id on quest_space(user_id);
...
create index quest_user_id on quest(user_id);
...
create index conn_space_quest_user_id on conn_space_quest(user_id);
create index conn_space_quest_quest_id on conn_space_quest(quest_id);
-- Словарь терминов
...
create index term_dict_user_id on term_dict(user_id);
...
create index conn_quest_term_user_id on conn_quest_term(user_id);
create index conn_quest_term_term_id on conn_quest_term(term_id);

-- Связь Пространство - Вопросы
create table conn_space_term
(
	...
	user_id bigint,	
	insert_ts timestamp default current_timestamp,
	constraint conn_space_term_space_id foreign key (space_id) references quest_space(id) on delete cascade,
	constraint conn_space_term_term_id foreign key (term_id) references term_dict(id) on delete cascade,
	constraint conn_space_term_user_id foreign key (user_id) references users(id),
	constraint un_conn_space_term unique (space_id, term_id)
);

comment on table conn_space_term is 'Таблица связей Пространство - Термины';
comment on column conn_space_term.id is 'Первичный ключ';
comment on column conn_space_term.space_id is 'Ссылка на пространство';
comment on column conn_space_term.term_id is 'Ссылка на термин';
comment on column conn_space_term.user_id is 'Ссылка на пользователя';
comment on column conn_space_term.insert_ts is 'Дата-время добавления записи';

create index conn_space_term_user_id on conn_space_term(user_id);
create index conn_space_term_term_id on conn_space_term(term_id);
